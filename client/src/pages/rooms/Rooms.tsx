...
import type { Room, RoomProps } from "@/types/types";
...
const Rooms = () => {
	...
	return (
		<PageLayout
			title="Explore Our Luxurious Rooms"
			...
			<Container>
				...
				>
					<TabButtons
						...
						onTabChange={(tab) => {
							if (tab === selectedTab) return; //prevent unnecessary api calls
							setSearchTerm(tab === "all rooms" ? {} : { type: tab });
						}}
					...
					<Grid container spacing={6} mt={4} sx={{ width: "100%" }}>
						{data === undefined || data.length === 0 ? (
							<Grid size={12}>
								<RoomsNotFound message={message} type={selectedTab} />
							</Grid>
						) : (
							data.slice(0, 6).map((room, index) => (
								<Grid key={room._id} size={{ xs: 12, sm: 12, md: 12 }}>
									<CCard index={index} orientation="vertical" room={room} />
								</Grid>
							))
						)}
					</Grid>
...
function RoomsNotFound({ message, type }: { message: string; type: string }) {
	return (
		<Box
			sx={{
				display: "flex",
